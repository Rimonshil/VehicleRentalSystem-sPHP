...
$EM = new EntityManagement($Table[$Entity = "PersonNote"]);
...
$EM->InputValidation([
	new HTTP\InputValidation("{$Entity}Message", true),
	//new HTTP\InputValidation("{$Entity}Number", true),
	new HTTP\InputValidation("PersonID", true),
...
$EM->ListColumn([
	new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Message") . "", "{$Caption}", null),
	//new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Number") . "", "{$Caption}", null),
//	new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "PersonID") . "", "{$Caption}", null),
	new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "TimeInserted") . "", "{$Caption}", FIELD_TYPE_SHORTDATE),
...
if(isset($_POST["btnInput"])){
	...
	$EM->InputUIHTML([
		HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Message") . "", $EM->InputWidth(), null, true), "{$Caption}", null, null, $EM->FieldCaptionWidth()),
		//HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Number") . "", $EM->InputWidth(), null, true), "{$Caption}", null, null, $EM->FieldCaptionWidth()),
		//HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Email") . "", $EM->InputWidth(), null, true), "{$Caption}", null, null, $EM->FieldCaptionWidth()),
...
//	SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}Number") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$Database->Escape($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"])}%'" : null,
	...
	SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}TimeInserted") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$Database->Escape($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"])}%'" : null,
...
if(isset($_POST["btnExport"])){
	...
	print $Table["{$Entity}"]->Export(
		"{$Entity}Type , {$Entity}Number,PersonFirstName",
		str_replace(" ", null, "Type,Number,PersonFirstName"),
