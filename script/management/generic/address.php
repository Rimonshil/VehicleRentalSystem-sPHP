...
$EM->ImportField([
    ...
    new Database\Field("{$Entity}Person" . ($Field = "ID") . "", "{$Field}"),
    new Database\Field("{$Entity}Country" . ($Field = "ID") . "", "{$Field}"),

	/*new Database\Field("{$Entity}First" . ($Field = "Name") . "", "{$Field}"),
	new Database\Field("{$Entity}" . ($Field = "LastName") . "", "{$Field}"),
...
$EM->InputValidation([
	...
	new HTTP\InputValidation("{$Entity}City", true),
	//new HTTP\InputValidation("{$Entity}BirthDate", true),
...
$EM->ListColumn([
    ...
    new HTML\UI\Datagrid\Column("" . ($Caption = "Person") . "FirstName", "{$Caption}", null),
	new HTML\UI\Datagrid\Column("" . ($Caption = "Country") . "Name", "{$Caption}", null),
...
if(isset($_POST["btnInput"])){
	...
	$EM->InputUIHTML([
		HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Type") . "", $EM->InputWidth(), null, true), "{$Caption}", null, null, $EM->FieldCaptionWidth()),
		HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Name") . "", $EM->InputWidth(), null, true), "{$Caption}", null, null, $EM->FieldCaptionWidth()),
		HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Street") . "", $EM->InputWidth(), null, true), "{$Caption}", null, null, $EM->FieldCaptionWidth()),
        HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "City") . "", $EM->InputWidth(), null, true), "{$Caption}", null, null, $EM->FieldCaptionWidth()),
        HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Zip") . "", $EM->InputWidth(), null, true), "{$Caption}", null, null, $EM->FieldCaptionWidth()),
		//HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "City") . "", $EM->InputWidth(), null, true,INPUT_TYPE_DATE), "{$Caption}", null, null, $EM->FieldCaptionWidth()),
		HTML\UI\Field(HTML\UI\Select("" . ($Caption = "Person") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get("{$Table["{$OptionEntity}"]->Alias()}.{$OptionEntity}IsActive = 1", "{$OptionEntity}LookupCaption ASC"), null, "{$OptionEntity}LookupCaption", null, null, null), "{$Caption}", true, null, $EM->FieldCaptionWidth()),
		HTML\UI\Field(HTML\UI\Select("" . ($Caption = "Country") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get("{$Table["{$OptionEntity}"]->Alias()}.{$OptionEntity}IsActive = 1", "{$OptionEntity}LookupCaption ASC"), null, "{$OptionEntity}LookupCaption", null, null, null), "{$Caption}", true, null, $EM->FieldCaptionWidth()),
...
$EM->SearchSQL([
	...
	SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}Street") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$Database->Escape($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"])}%'" : null,
    SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}City") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$Database->Escape($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"])}%'" : null,
	SetVariable("{$Configuration["SearchInputPrefix"]}" . ($Column = "{$Entity}Zip") . "", SetVariable($Column)) ? "{$Table["{$Entity}"]->Alias()}.{$Column} LIKE '%{$Database->Escape($_POST["{$Configuration["SearchInputPrefix"]}{$Column}"])}%'" : null,
...
$EM->SearchUIHTML([
	HTML\UI\Field(HTML\UI\Input("{$Configuration["SearchInputPrefix"]}{$Entity}" . ($Caption = "Type") . "", 200), "{$Caption}", null, true),
   ...
   // HTML\UI\Field(HTML\UI\Input("{$Configuration["SearchInputPrefix"]}{$Entity}" . ($Caption = "Zip") . "", 200), "{$Caption}", null, true),
	...
	HTML\UI\Field(HTML\UI\Select("{$Configuration["SearchInputPrefix"]}" . ($Caption = "Person") . "ID", $Table[$OptionEntity = "{$Caption}"]->Get(null, "" . ($OptionEntityOrderBy = "{$OptionEntity}LookupCaption") . " ASC"), new Option(), "{$OptionEntityOrderBy}"), "{$Caption}", null, true),
...
if(isset($_POST["btnExport"])){
	...
	print $Table["{$Entity}"]->Export(
		"{$Entity}Type , {$Entity}Name,{$Entity}Street,{$Entity}City,{$Entity}Zip,PersonFirstName",
		str_replace(" ", null, "Type,Name,Street,City,Zip,UserName"),
